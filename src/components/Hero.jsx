...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    supabase.from('hero_stats').select('*').order('sort_order')
      .then(({ data }) => { if (data && data.length) setStats(data); }).catch(() => {});
    supabase.from('hero_emojis').select('*').order('float_delay')
      .then(({ data }) => { if (data && data.length) setEmojis(data); }).catch(() => {});
    supabase.from('animation_settings').select('*').eq('component', 'hero').single()
      .then(({ data }) => {
        if (data) {
          setOrbEnabled(data.orb_enabled !== false);
          setGridEnabled(data.grid_enabled !== false);
          setTdEnabled(data.three_d_enabled !== false);
        }
      }).catch(() => {});
  }, []);
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{
          rotateX: tdEnabled ? rotX : 0,
          rotateY: tdEnabled ? rotY : 0,
      ...
      >
        ...
        >
          ...
          <motion.a
            ...
            className="px-8 py-4 text-white font-bold rounded-2xl text-lg backdrop-blur-sm cursor-pointer"
          >
